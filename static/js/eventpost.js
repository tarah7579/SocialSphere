    ...
    function handleEditClick(event) {
        ...
        const captionElement = eventElement.querySelector('.event-caption');
        ...
        // Use the full caption from data-fulltext and replace <br> with newline characters
        const fullCaption = captionText.getAttribute('data-fulltext');
        const originalTitle = title.textContent;
        const originalCaption = (fullCaption || captionText.textContent).replace(/<br\s*\/?>/gi, '\n');  // Convert <br> to newline
        const originalPhoto = photo ? photo.innerHTML : '';
    
        // Replace content with input fields for editing
        title.innerHTML = `<input type="text" value="${originalTitle}" id="edit_title_${eventId}">`;
        captionElement.innerHTML = `<textarea id="edit_caption_${eventId}">${originalCaption}</textarea>`;
    
        if (photo) {
            photo.innerHTML = `<input type="file" id="edit_photo_${eventId}" accept="image/*">`;
        }
        ...
        // Save original handlers and set new ones for Save and Cancel
        button.removeEventListener('click', handleEditClick);
        button.addEventListener('click', (e) => handleSaveClick(e, originalTitle, originalCaption, originalPhoto, deleteBtn));
    
        cancelButton.addEventListener('click', () => {
            location.reload(); // Refresh the page to reset everything, including "See More"
        });
    }
    ...
    function toggleOtherEventTypeInput(selectedValue) {
        ...
        if (selectedValue === 'Others') {
            otherInput.style.display = 'block'; // Show the input if 'Others' is selected
        } else {
            otherInput.style.display = 'none';  // Hide the input otherwise
        }
    }
