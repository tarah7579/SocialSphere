...
def post_to_facebook(event):
    try:
        ...
        post_data = {
            ...
            'published': True,
            'scheduled_publish_time': timezone.now().timestamp()  # Adjust if you want future scheduling
        }

        # Convert the binary image data to a file-like object using BytesIO
        image_file = io.BytesIO(event.photo)

        # Post the photo to Facebook with the event's title and caption
        graph.put_photo(image=image_file, message=event.caption)
